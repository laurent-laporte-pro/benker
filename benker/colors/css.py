...
#: CSS color names (and RGB values), see: `<https://www.w3.org/TR/css-color-3/#svg-color>`_
CSS_COLOR_NAMES = {
...
def css_name_to_rgb(text, rgb_scale=RGB_SCALE):
    color_name = parse_css_name(text)
...
def css_name_to_hsl(text, hue_scale=HUE_SCALE):
    color_name = parse_css_name(text)
    r, g, b = CSS_COLOR_NAMES[color_name.lower()]
    h, s, l = rgb_to_hsl(r, g, b, hue_scale=hue_scale)
    return h, s, l
...
def css_name_to_cmyk(text, cmyk_scale=CMYK_SCALE):
    color_name = parse_css_name(text)
    r, g, b = CSS_COLOR_NAMES[color_name.lower()]
    c, m, y, k = rgb_to_cmyk(r, g, b, cmyk_scale=cmyk_scale)
    return c, m, y, k
